...
# Select a single machine (e.g., machine with machineID=1)
machine_id = 1

# Prepare the training data for the selected machine
comp_failures = failures[failures['machineID'] == machine_id]
comp_maint = maint[maint['machineID'] == machine_id]
...
for _, row in comp_maint.iterrows():
    ...
    prev_failures = comp_failures[
        (comp_failures['failure'] == component) &
        (comp_failures['datetime'] < maint_date)
    ]['datetime'].tolist()
    prev_maint = comp_maint[
        (comp_maint['comp'] == component) &
...
# Make predictions for the selected machine
latest_telemetry = telemetry[telemetry['machineID'] == machine_id].iloc[-1]

# Find the previous failure and maintenance events for this machine
prev_failures = comp_failures['datetime'].tolist()
prev_maint = comp_maint['datetime'].tolist()

# Iterate over the components and make predictions
for component in comp_maint['comp'].unique():
    # Filter the previous failures and maintenance events for the current component
    comp_prev_failures = comp_failures[comp_failures['failure'] == component]['datetime'].tolist()
    comp_prev_maint = comp_maint[comp_maint['comp'] == component]['datetime'].tolist()
    
    # Compute the time since the last failure and maintenance for the current component
    time_since_failure = (latest_telemetry['datetime'] - max(comp_prev_failures, default=pd.Timestamp(1970, 1, 1))) / pd.Timedelta(days=1) if comp_prev_failures else 0
    time_since_maint = (latest_telemetry['datetime'] - max(comp_prev_maint, default=pd.Timestamp(1970, 1, 1))) / pd.Timedelta(days=1) if comp_prev_maint else 0
    ...
    print(f"Machine ID: {machine_id}, Component: {component}, Remaining days: {int(remaining_days)}")